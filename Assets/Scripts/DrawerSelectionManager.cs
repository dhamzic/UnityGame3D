...
public class DrawerSelectionManager : MonoBehaviour
{
    [SerializeField] private string selectableTag = "DrawerSelectable";
    ...
    public AnimController drawerAnimationScript;
        ...
        if (Physics.Raycast(ray, out hit))
        {
            if (hit.distance <= 6)
            {
                ...
                if (selection.CompareTag(selectableTag))
                {
                    ...
                    {
                        drawerAnimationScript.StartDrawerAnimation(hit.transform.name);
                    }
